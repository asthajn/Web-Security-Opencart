<?php
class ControllerSaleReferee extends Controller {
	public function index() {     
		$this->language->load('sale/referee');
		...
		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('sale/referee', 'token=' . $this->session->data['token'] . $url, 'SSL'),
      		'separator' => ' :: '
   		);		
...
$referee = $this->model_report_customer->getRefereeDetails();

foreach ($referee as $ref){
$this->data['refererrs'][] = array(
				'referee'       => $ref['referrer_id'],
				'referals'      => $ref['count(referrer_id)'],
'href1'		=>$this->url->link('sale/referee_detail/', 'token=' . $this->session->data['token'] . '&referrer_id=' . $ref['referrer_id'] . $url, 'SSL')
				);
}
/*		
...
//$this->data['customer_order1'] = $this->url->link('report/customer_data', 'token=' . $this->session->data['token'] . '&customer_id=' . $result['customer_id']. $url, 'SSL');	
		...
		$this->data['column_customer'] = $this->language->get('column_referee');
		$this->data['column_email'] = $this->language->get('column_referals');
		...
		$pagination->url = $this->url->link('sale/referee', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->template = 'sale/referee.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
