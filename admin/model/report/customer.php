...
$query = $this->db->query("SELECT referrer_id, count(referrer_id) from oc_order where referrer_id <> '' group by referrer_id");

return $query->rows;
}

public function getIndividualDetails($ref_id){

$query = $this->db->query("select order_id, product_id, name, model, total from oc_order_product where order_id IN (select order_id from oc_order where referrer_id <> '')");
